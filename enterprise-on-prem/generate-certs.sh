...
# per-component topology uses the rest.
HOSTS=(
  3lc.localhost
...
openssl req -x509 -newkey rsa:4096 -sha256 -days ${DAYS_CA} -nodes   -keyout ca.key -out ca.crt   -subj "/CN=3LC Enterprise On-Prem deployment example local CA/O=3LC/OU=Examples"   -addext "basicConstraints=critical,CA:TRUE"   -addext "keyUsage=critical,keyCertSign,cRLSign"
